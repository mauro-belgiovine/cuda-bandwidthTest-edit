...
std::vector<double> measure_vector_y, measure_vector_x;
...
//#define DEFAULT_SIZE        ( 1536 * ( 1 << 20 ) )    //1.5 GB
...
void plot_results(std::vector<double> measure_vector_y, std::vector<double> measure_vector_x, std::string plot_title, std::string output_file_label, double average, double std_deviation);
double get_average(std::vector<double> measure_vector);
double get_standard_deviation(std::vector<double> measure_vector, double average);
...
int runTest(const int argc, const char **argv)
{
    uint start = DEFAULT_SIZE;
    uint end = DEFAULT_SIZE;
    int startDevice = 0;
    int endDevice = 0;
    ...
    uint iter = 1; //default is single iteration
    ...
    if (checkCmdLineFlag(argc, (const char **)argv, "iter")){
    	...
    	iter = getCmdLineArgumentInt(argc, argv, "iter");
        if (iter <= 0){
        	printf("Illegal argument - iteration (iter) must be greater than zero\n");
            return -11000;
        }
    }

    //redirect stdout to a file named output-getpid.txt
    std::stringstream outname;
    outname << "output-" << getpid() << ".txt";
    freopen(outname.str().c_str(), "w", stdout);
    ...
    if (htod)
    {
    	...
    	//average
    	double avg = get_average(measure_vector_y);
    	printf("The average bandwidth over total transfers is: %f\n", avg);
    	//standard deviation
    	double std_dev = get_standard_deviation(measure_vector_y, avg);
    	printf("The standard deviation of bandwidth over total transfers is: %f\n", std_dev);
        //plot
        plot_results(measure_vector_y, measure_vector_x, "Host -> Device Bandwidth (MB/s)", "htod_output", avg, std_dev);
        measure_vector_y.clear();
        measure_vector_x.clear();
        printf("----------------------------------------------\n\n");
    }
    ...
    if (dtoh)
    {
    	...
    	printf("The standard deviation of bandwidth over total transfers is: %f\n", std_dev);
    	//plot
    	plot_results(measure_vector_y, measure_vector_x, "Device -> Host Bandwidth (MB/s)", "dtoh_output", avg, std_dev);
    	measure_vector_y.clear();
    	measure_vector_x.clear();
    	printf("----------------------------------------------\n\n");
    ...
    if (dtod)
    {
    	...
    	printf("The standard deviation of bandwidth over total transfers is: %f\n", std_dev);
    	//plot
        plot_results(measure_vector_y, measure_vector_x, "Device -> Device Bandwidth (MB/s)", "dtod_output", avg, std_dev);
    	measure_vector_y.clear();
    	measure_vector_x.clear();
    	printf("----------------------------------------------\n\n");
                   ...
                   memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc)
{
    ...
    for (int currentDevice = startDevice; currentDevice <= endDevice; currentDevice++)
    {
        ...
        for (unsigned int i = 0; i < count; i++)
        {
            ...
            //save bandwidth data in global vector
            measure_vector_y.push_back(bandwidths[i]);
            //if it is a RANGE_MODE transfer
            if(start != end){
            	measure_vector_x.push_back(memSizes[i]);
            }
        }
...
void plot_results(std::vector<double> measure_vector_y, std::vector<double> measure_vector_x, std::string plot_title, std::string output_file_label, double average, double std_deviation){
	...
	Gnuplot g;
	if(measure_vector_x.size() > 0){
		g.plot_xy(measure_vector_x, measure_vector_y, plot_title);
	}else{
		g.plot_x(measure_vector_y, plot_title);
	}
	g.set_xlabel("Indice trasferimento (x10)");
	g.set_ylabel("Bandwidth (MB/s)");
	g.set_smooth().plot_x(measure_vector_y,"cspline");
	std::stringstream plot_header;
	plot_header << plot_title << " --- Average bandwidth: " << average << " - Standard deviation: " << std_deviation;
	g.set_title(plot_header.str());
	std::time_t t = std::time(0);
	std::stringstream filename;
	filename << output_file_label << "-" << t << "-" << getpid();
	g.savetops(filename.str());
	if(measure_vector_x.size() > 0){
		g.set_smooth("bezier").plot_xy(measure_vector_x, measure_vector_y,"bezier");
	}else{
		g.set_smooth("bezier").plot_x(measure_vector_y,"bezier");
	}
	//g.showonscreen();
	//g.unset_smooth();
}
...
double get_average(std::vector<double> measure_vector){
	double avg = 0;
	for(uint i = 0; i < measure_vector.size(); ++i) avg += measure_vector.at(i);
	avg = avg / measure_vector.size();
	return avg;
}
...
double get_standard_deviation(std::vector<double> measure_vector, double average){
	double std_dev = 0;
	double numerator = 0;
	double denominator = measure_vector.size();

	for(uint i = 0; i < measure_vector.size(); ++i) numerator = numerator + pow((measure_vector.at(i) - average), 2);

	std_dev = sqrt(numerator/denominator);
