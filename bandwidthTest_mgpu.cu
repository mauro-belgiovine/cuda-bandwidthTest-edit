...
float testDeviceToHostTransfer_mgpu(unsigned int num_gpus, unsigned int memSize, memoryMode memMode, bool wc);
                   ...
                   memcpyKind kind, printMode printmode, memoryMode memMode, int startDevice, int endDevice, bool wc)
{
    ...
    /*for (int currentDevice = startDevice; currentDevice <= endDevice; currentDevice++)
    {
        ...
        for (unsigned int i = 0; i < count; i++)
        {
            ...
            {
                case DEVICE_TO_HOST:*/
                    bandwidths[i] += testDeviceToHostTransfer_mgpu(((endDevice - startDevice)+1),memSizes[i], memMode, wc);
                    /*break;
...
//BEGIN EDIT ---- belgiovi
///////////////////////////////////////////////////////////////////////////////
//! test the bandwidth of a device to host multi-GPU memcopy of a specific size
//! using indipendent stream per device
///////////////////////////////////////////////////////////////////////////////
float
testDeviceToHostTransfer_mgpu(unsigned int num_gpus, unsigned int memSize, memoryMode memMode, bool wc)
{
    ...
    float bandwidthInMBs = 0.0f;
    unsigned char *h_idata = NULL;
    unsigned char *h_odata[num_gpus];
    cudaEvent_t start, stop;
    cudaStream_t stream[num_gpus];
    
#ifdef DOUBLE_STREAM    
    /* HARDCODED: dati per testare doppio stream su GPU Kepler K20m (solo PINNED MEMORY)*/
    cudaStream_t stream2[num_gpus];
    unsigned char *h_odata2[num_gpus];
#endif
    
    for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	h_odata[currentDevice] = NULL;
    }
...
#if CUDART_VERSION >= 2020
        checkCudaErrors(cudaHostAlloc((void **)&h_idata, memSize, (wc) ? cudaHostAllocWriteCombined : 0));
	//alloca area di memoria (output) su memoria Host per ogni GPU
        for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    checkCudaErrors(cudaHostAlloc((void **)&h_odata[currentDevice], memSize, (wc) ? cudaHostAllocWriteCombined : 0));
	    
	    if (h_odata[currentDevice] == 0)
	    {
		fprintf(stderr, "Not enough memory avaialable on host to run test!\n");
		exit(EXIT_FAILURE);
	    }
...
#ifdef DOUBLE_STREAM
	for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    checkCudaErrors(cudaHostAlloc((void **)&h_odata2[currentDevice], memSize, (wc) ? cudaHostAllocWriteCombined : 0));
	    
	    if (h_odata2[currentDevice] == 0)
	    {
		fprintf(stderr, "Not enough memory avaialable on host to run test!\n");
		exit(EXIT_FAILURE);
	    }
	...
	//alloca area di memoria (output) su memoria Host per ogni GPU
        for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    checkCudaErrors(cudaMallocHost((void **)&h_odata[currentDevice], memSize));
...
#ifdef DOUBLE_STREAM
	for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    checkCudaErrors(cudaMallocHost((void **)&h_odata2[currentDevice], memSize));
	...
	//alloca area di memoria (output) su memoria Host per ogni GPU
        for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    h_odata[currentDevice] = (unsigned char *)malloc(memSize);
    ...
    unsigned char *d_idata[num_gpus];
    
#ifdef DOUBLE_STREAM
    unsigned char *d_idata2[num_gpus];
#endif
    
    //inizializziamo gli stream per ogni device e allochiamo la memoria sulle GPU
    for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	
	checkCudaErrors(cudaSetDevice(currentDevice));
	
	checkCudaErrors(cudaStreamCreate(&stream[currentDevice]));
	// allocate device memory
	checkCudaErrors(cudaMalloc((void **) &d_idata[currentDevice], memSize));
	
	//initialize the device memory
	checkCudaErrors(cudaMemcpy(d_idata[currentDevice], h_idata, memSize,
                               cudaMemcpyHostToDevice));
	
#ifdef DOUBLE_STREAM
	checkCudaErrors(cudaStreamCreate(&stream2[currentDevice]));
	// allocate device memory
	checkCudaErrors(cudaMalloc((void **) &d_idata2[currentDevice], memSize));
	
	//initialize the device memory
	checkCudaErrors(cudaMemcpy(d_idata2[currentDevice], h_idata, memSize,
                               cudaMemcpyHostToDevice));
#endif	
    ...
    //copiamo i dati da ogni GPU sulla memoria Host in aree separate
    for (int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	...
	if (PINNED == memMode)
	{
	    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
	    {
		checkCudaErrors(cudaMemcpyAsync((void *) h_odata[currentDevice], (const void *) d_idata[currentDevice], memSize,
						cudaMemcpyDeviceToHost, stream[currentDevice]));
	    }
	    
#ifdef DOUBLE_STREAM
	    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
	    {
		checkCudaErrors(cudaMemcpyAsync((void *) h_odata2[currentDevice], (const void *) d_idata2[currentDevice], memSize,
						cudaMemcpyDeviceToHost, stream2[currentDevice]));
	    }
#endif
	}
	else
	{
	    for (unsigned int i = 0; i < MEMCOPY_ITERATIONS; i++)
	    {
		checkCudaErrors(cudaMemcpy(h_odata[currentDevice], d_idata[currentDevice], memSize,
					  cudaMemcpyDeviceToHost));
	    }
	}
    }
    ...
    for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	checkCudaErrors(cudaStreamSynchronize(stream[currentDevice]));	
    }
    
#ifdef DOUBLE_STREAM
    for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	checkCudaErrors(cudaStreamSynchronize(stream2[currentDevice]));	
    }
#endif
    ...
    //checkCudaErrors(cudaEventRecord(stop, 0));
    // Sync events (BLOCKS till last (stop in this case) has been recorded!)
...
#ifdef DOUBLE_STREAM
    //calculate bandwidth in MB/s
    bandwidthInMBs = (1e3f * (memSize*2) * (float)MEMCOPY_ITERATIONS * num_gpus) /
                     (elapsedTimeInMs * (float)(1 << 20));
#else
    //calculate bandwidth in MB/s
    bandwidthInMBs = (1e3f * memSize * (float)MEMCOPY_ITERATIONS * num_gpus) /
                     (elapsedTimeInMs * (float)(1 << 20));
	...
	for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    checkCudaErrors(cudaFreeHost(h_odata[currentDevice]));
	}
#ifdef DOUBLE_STREAM
	for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	    checkCudaErrors(cudaFreeHost(h_odata2[currentDevice]));	
	}
#endif	
    }
    else
    {
        free(h_idata);
	for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	  free(h_odata[currentDevice]);
	}
    }
    
    for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	checkCudaErrors(cudaFree(d_idata[currentDevice]));
	checkCudaErrors(cudaStreamDestroy(stream[currentDevice]));
    }

#ifdef DOUBLE_STREAM
    for(int currentDevice = 0; currentDevice < num_gpus; currentDevice++){
	checkCudaErrors(cudaFree(d_idata2[currentDevice]));
	checkCudaErrors(cudaStreamDestroy(stream2[currentDevice]));
    }
#endif
